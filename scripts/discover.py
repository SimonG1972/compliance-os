...
try:
    ...
    from src.tools.normalize_policy import (
        normalize_url_with_policy,
        allowed_by_policy_path,
        suggest_normalized_url,
    )
...
def insert_urls(con, urls, mode: str, discovered_from: str) -> int:
    ...
    ins = 0
    for u in urls:
        # === BEGIN POLICY HOOK (shadow-safe normalization and allow/deny) ===
        if _POLICY_AVAILABLE:
            try:
                _pol = get_policy_for_url(u)
                _suggested = suggest_normalized_url(u, _pol)

                if _suggested != u:
                    print(f"[policy-shadow] {u}  ->  {_suggested}")

                _allowed = allowed_by_policy_path(_suggested, _pol)
                if not _allowed:
                    print(f"[policy-shadow-deny] {u}")
                    if policy_enforce_enabled():
                        # skip enqueue when enforcement is ON
                        continue

                # normalize (returns original u when enforcement is OFF)
                u = normalize_url_with_policy(u, _pol)

            except Exception:
                # Never break discovery due to policy layer
                pass
        # === END POLICY HOOK ===
